  ...
  return (
    <aside className="w-full max-w-xs bg-white rounded-2xl shadow-lg p-6 flex flex-col gap-6 sticky top-16 mt-12">
      ...
      <details open className="group">
        ...
        <div className="flex flex-wrap gap-2 mt-2 pl-1">
          {tamanhos.map((tamanho) => (
            <label key={tamanho} className="flex items-center gap-1 cursor-pointer">
              <input type="checkbox" className="accent-zinc-950 rounded" />
              <span className="text-zinc-700 text-sm">{tamanho}</span>
            </label>
          ))}
        </div>
      </details>
      ...
      <details open className="group">
        ...
        <div className="flex flex-col gap-2 mt-2 pl-1">
          <input type="range" min={100} max={1500} step={50} className="w-full accent-zinc-950" />
      ...
      <button
        ...
        style={selectedMarcas.length === 0 ? { opacity: 0.6, cursor: 'not-allowed' } : {}}
      >
        Aplicar filtros
      </button>
    </aside>
